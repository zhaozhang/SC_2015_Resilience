...
\subsection*{Abstract}
Resilience is critical for applications running
on distributed computing platforms in which node failures may occur frequently.
When replaying a failed task, we may rely on online backups either of a task's input
data or of lineage metadata describing how that data were computed.
However, applying either technique blindly can be inefficient due to either
excessive recovery costs if only lineage is recorded or excessive data replication costs if all data is replicated.
We present an adaptive backup approach that uses models of backup and
recovery costs to guide decisions regarding backup methods, with the goal of identifying
at run time the choices that work best for a given computer system and application.
We implement this adaptive backup approach inside the AMFORA parallel scripting framework
and evaluate its effectiveness with a scientific image processing application, Montage.
When running this application on 64 compute nodes, we find that the adaptive approach
recovers with better performance (up to 57\% better) than
do pure re-execution or replication approaches, while introducing only 2.9\% overhead in failure-free performance.
