...
\begin{abstract}
Resilience is critical for many-task applications running on computing platforms with node failures, especially when using transient in-memory file system for parallel scripting. The state of the runtime system is in the form of in-memory file system status.
Existing systems overcome node failure by resilient data access either through task re-execution or additional replicas. 
\katznote{prev is too blunt/general - if you are going to frame this as resilient data access, you first need to talk about the model you are using - data is primary, computing is secondary - overall state is stored via data - tasks are generally somewhat short-running - etc.} 
\zhaonote{add a few words in the begging and also the sentence after this comment, it is more than 150 words now, but we can work on the word limit later.}
However, applying either technique blindly can be inefficient due to 
excessive recovery or data replication costs due to the irregularity of task runtimes and I/O.
...
In this paper, we make the following assumptions: The network that connects nodes has a uniform bandwidth and latency;  \katznote{why do we do this?  What happens if this is not true (which is almost certainly the case)} 
\zhaonote{We assume this to simplify the discussion. Also for the system, we require user to specify the bandwidth and latency as for system configuration. And we show that this works fine for performance in practice.}
node failure means that the data stored in that node is not accessible; and lineage information gathering and data replication are synchronous. To simplify the discussion, we also assume that all nodes share a common, consistent failure rate and failure are uncorrelated. But later we discover the failure has little impact on the adaptive backup model in Section~\ref{sec:Perf:Failure}. 
...
$(1-P)^nT$. 
\katznote{if all of the nodes are available, where does P come in?}  
\zhaonote{fixed the above text.}
...
\label{sec:Impl}
To integrate the adaptive backup model into the AMFORA framework, we implemented two backup primitives: replicate() and lineage(). The {\em replicate()} primitive creates a configurable number of replicas for a file on peer nodes. The {\em lineage()} primitive records a file's lineage (i.e., the command line used to produce it) in the file's metadata. Also recorded in the file metadata are the file's host address and expected recovery cost. The execution engine captures task runtime information such as time-to-solution and I/O size. It also has to 
\katznote{it needs to, or it does?} 
\zhaonote{``it has to'' may be more accurate, because we nee this piece of information to make the adaptive approach work.}
identify what parameters in the command line are input files and which are output files.
...
\katznote{each local metadata and data cache? what does this mean} 
\zhaonote{it should be without cache, fixed.}
